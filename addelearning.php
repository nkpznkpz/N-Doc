...
$file2=true;
	//echo "llll".$_FILES['picfile']['type'];
	 if(!$_FILES['picfile']['name']){
				...
				if(!check_filetype($_FILES['picfile']['type'],$type_img_check)){
						$errortext = " ไฟล์ ".$_FILES['picfile']['name']."ไม่ใช่ไฟล์รูปที่กำหนดไว้<br>";
						$iserror=1;
				}else if($_FILES['picfile']['size']>512000){
						$errortext .="  ไฟล์ ".$_FILES['picfile']['name']."ขนาดเกินกำหนด<br>";		
						$iserror=1;
				}
	...
	if(!$_FILES['file1']['name']){
				...
				if(!check_filetype($_FILES['file1']['type'],$type_file_check)){
						$errortext .= " ไฟล์ ".$_FILES['file1']['name']." ไม่ใช่ไฟล์เอกสาร<br>";
						$iserror=1;
				}else{
				...
				if($_FILES['file1']['size']>512000){
						$errortext .="  ไฟล์ ".$_FILES['file1']['name']."  ขนาดเกินกำหนด<br>";		
						$iserror=1;
				}else{
	...
	if(!$_FILES['file2']['name']){
				...
				if(!check_filetype($_FILES['file2']['type'],$type_file_check)){
						$errortext .= " ไฟล์ ".$_FILES['file2']['name']."ไม่ใช่ไฟล์เอกสาร<br>";
						$iserror=1;
				}
				if($_FILES['file2']['size']>512000){
						$errortext .="  ไฟล์ ".$_FILES['file2']['name']."ขนาดเกินกำหนด<br>";		
						$iserror=1;
				}
	}
			...
			if($iserror==1){
					echo  $errortext;
					echo '<a href="javascript:back();">กลับไปแก้ไข</a>';
			}else{
				...
				//save file
						...
						echo "คุณได้เพิ่มสื่อการเรียนรู้เรียบร้อยแล้ว กรุณารอการยืนยันสื่อการเรียนรู้นี้จาก webmaster  ครับ";
		}
			//check file
